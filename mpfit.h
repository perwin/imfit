 ...
 * making it difficult to use this with a C++ program!); added standard 
 * #ifdef __cplusplus boilerplate wrapper.
 */
...
typedef int (*mp_func)(int m, /* Number of functions (elts of fvec) */
		       ...
		       void *privateData); /* I/O - function private data*/
...
extern int mpfit(mp_func funct, int m, int npar,
		 double *xall, mp_par *pars, mp_config *config, void *privateData, 
		 mp_result *result);
