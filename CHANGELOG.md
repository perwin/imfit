...
## 1.3 -- 2015-xx-xx
### Added: 
- New image function: modified (or "empirical") King profile, as described in
Elson et al. (1999) and Peng et al. (2010).
...
- PSF convolution code has been rewritten to use smaller arrays and slightly faster
...
a speedup in doing PSF convolutions (see below).
...
### Fixed:
- Convolutions when the model contains negative pixels are now done correctly (even
though models with negative pixels are generally not a good idea...).
